...
- All active hypotheses in the Hypothesis Tracker are CONFIRMED or REFUTED
- 2+ backends with **confirmed instrumentation** return no anomaly AND no new hypothesis would be generated by querying a third backend
- Further queries would be speculative (no hypothesis they could distinguish between)

**CONTINUE only when** ALL of the following are true:
- ≥1 hypothesis remains ACTIVE in the Hypothesis Tracker
- A specific backend query would change that hypothesis's status
- Name the backend and the query before proceeding — if you can't, STOP

**Instrumentation gap exception:** If a backend returned empty AND discovery attempts suggest it may not be instrumented for this service → mark as `[INSTRUMENTATION_GAP]` in Hypothesis Tracker, do NOT count it as a "no anomaly" backend.
